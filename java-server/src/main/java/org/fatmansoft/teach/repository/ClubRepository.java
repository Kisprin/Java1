...
public interface ClubRepository extends JpaRepository<Club,Integer> {
    Optional<Club> findClubByClubId(Integer clubId);

    @Query(value = "from Club where ?1='' or clubNum = ?1  ")
    Optional<Club> findClubByClubNum(String clubNum);

    @Query(value = "from Club where ?1='' or clubNum like %?1% or clubName like %?1% ")
    List<Club> findClubListByNumName(String numName);
