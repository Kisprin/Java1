...
public class OutlearnController extends ToolController {
    @FXML
    ...
    @FXML
    private TableColumn<Map,String> reasonColumn; //学生信息表 头衔列
    @FXML
    private TableColumn<Map,String> seColumn; //学生信息表 奖项列
    ...
    @FXML
    private TextField reasonField;  //学生信息  头衔输入域
    @FXML
    private TextField seField;   //学生信息  奖项输入域

    @FXML
    private TextField numNameTextField;  //查询 姓名学号输入域

    private Integer studentId = null;  //当前编辑修改的学生的主键
    private Integer outlearnId = null;
    ...
    public void initialize() {
        ...
        req.add("numName","");
        res = HttpRequestUtil.request("/api/outlearn/getOutlearnList",req); //从后台获取所有学生信息列表集合
        ...
        seColumn.setCellValueFactory(new MapValueFactory<>("se"));
    ...
    protected void changeStudentInfo() {
        ...
        DataResponse res = HttpRequestUtil.request("/api/outlearn/getOutlearnInfo", req);
        if (res.getCode() != 0) {
    ...
    protected void onSaveButtonClick () {
        ...
        req.add("studentId", studentId);
        req.add("form", form);
        DataResponse res = HttpRequestUtil.request("/api/outlearn/outlearnEditSave", req);
        if (res.getCode() == 0) {
    ...
    protected void onQueryButtonClick () {
        ...
        DataResponse res = HttpRequestUtil.request("/api/outlearn/getOutlearnList", req);
        if (res != null && res.getCode() == 0) {
            studentList = (ArrayList<Map>) res.getData();
            setTableViewData();
        }
    ...
    protected void onDeleteButtonClick () {
        ...
        outlearnId = CommonMethod.getInteger(form, "outlearnId");
        DataRequest req = new DataRequest();
        req.add("outlearnId", outlearnId);
        DataResponse res = HttpRequestUtil.request("/api/outlearn/outlearnDelete", req);
        if (res.getCode() == 0) {
